/*
    ...
    参数: {
        ...
        moveStart: function(){}, // 可选，Function类型，移动开始时的回调，默认会返回一个信息对象，包括当前对象obj，当前对象的序列号idx，当前对象的宽度width、当前对象的高度height、当前对象的左偏移left、当前对象的上偏移top。
        moving: function(){}, // 可选，Function类型，移动过程中的回调，返回信息同上
        moveEnd: function(){}, // 可选，Function类型，移动结束时的回调，返回信息同上
        zoomStart: function(){}, // 可选，Function类型，缩放开始时的回调，返回信息同上
        zooming: function(){}, // 可选，Function类型，缩放过程中的回调，返回信息同上
        zoomEnd: function(){} // 可选，Function类型，缩放结束时的回调，返回信息同上
...
(function($, doc) {
    ...
    ZoomAndMove.prototype.init = function() {
        if (this.opts && this.opts.isZoom===true) {
            var bars = [],
                dirArr = ["lt","ct","rt","lc","rc","lb","cb","rb"],
                barSize = 5,
                barZindex = 2;
            ...
            for (var i=0, count=dirArr.length; i<count; i++) {
                ...
                _dragZoom(this.$el, this.idx, dirArr[i], this.opts.zoomStart, this.opts.zooming, this.opts.zoomEnd, this.opts.always, $(this.opts.container));
            }
        ...
        if (this.opts && this.opts.isMove===true) {
            _dragMove(this.$el, this.idx, this.opts.dragbar, this.opts.moveStart, this.opts.moving, this.opts.moveEnd, this.opts.always, $(this.opts.container));
        }
    }

    function _dragZoom($el, idx, direc, zoomStart, zooming, zoomEnd, always, $container) {
        var point = {x:0, y:0},
            isZoom = false;
        ...
        $el.on("mousedown", "."+direc, function(e) {
            ...
            var baseWidth = parseInt($el.css("width")),
                baseHeight = parseInt($el.css("height")),
                container_top = _getPos(document.body).top,
                container_left = _getPos(document.body).left,
                ...
                el_top = _getPos($el[0]).top,
                el_left = _getPos($el[0]).left,
            ...
            if ($container.length) {
                container_top = _getPos($container[0]).top;
                container_left = _getPos($container[0]).left;
            ...
            var diff_top =  el_top-container_top,
            ...
            $(doc).off("mousemove").on("mousemove", function(e) {
                ...
                if (isZoom) {
                    switch(direc) {
                        case "lt": {
                            var width = baseWidth+point.x-e.clientX,
                        ...
                        break;
                        case "ct": {
                            var height = baseHeight+point.y-e.clientY,
                                top = baseTop+e.clientY-point.y;
                        ...
                        case "lb": {
                            var width = baseWidth+point.x-e.clientX,
                                height = baseHeight+e.clientY-point.y,
                                left = baseLeft+e.clientX-point.x;
                ...
                if (isZoom && isZooming && typeof zoomEnd == 'function') {
                    zoomEnd({
                        obj: $el,
                        idx: idx,
    ...
    function _dragMove($el, idx, dragbar, moveStart, moving, moveEnd, always, $container) {
        var point = {x:0, y:0},
            isMove = false,
        ...
        $dragBar.on("mousedown", function(e) {
            ...
            var baseTop = isNaN(parseInt($el.css("top"))) ? diff_top : parseInt($el.css("top")),
                baseLeft = isNaN(parseInt($el.css("left"))) ? diff_left : parseInt($el.css("left")),
            ...
            $(doc).off("mousemove").on("mousemove", function(e) {
                ...
                var left = baseLeft+e.clientX-point.x,
                    top = baseTop+e.clientY-point.y;
                ...
                if (isMove) {
                    ...
                    if( top-baseTop !=0 || left-baseLeft !=0 ){
                        isMoving = true;

                        if (moving && typeof moving == 'function') {
                            moving({
                                obj: $el,
                                idx: idx,
                                width: baseWidth,
                ...
                if (isMove && isMoving && typeof moveEnd == 'function') {
                    moveEnd({
                        obj: $el,
                        idx: idx,
                        width: baseWidth,
